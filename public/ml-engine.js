...
class DataCollector {
  ...
  record(readings) {
    ...
    const entry = {
      ...
      S_PH: this._normalizeValue(readings.phTanah),
      S_EC: this._normalizeValue(readings.ecTanah),
      S_N: this._normalizeValue(readings.nTanah),
      S_P: this._normalizeValue(readings.pTanah),
      S_K: this._normalizeValue(readings.kTanah),
      S_T: this._normalizeValue(readings.suhuTanah),
      S_H: this._normalizeValue(readings.kelembapanTanah),
    };
...
function extractFeatures(records) {
  ...
  const S_PH = records.map((r) => r.S_PH);
  const S_EC = records.map((r) => r.S_EC);
  const S_N = records.map((r) => r.S_N);
  const S_P = records.map((r) => r.S_P);
  const S_K = records.map((r) => r.S_K);
  const S_T = records.map((r) => r.S_T);
  const S_H = records.map((r) => r.S_H);
  ...
  return {
    ...
    soilPh: { mean: mean(S_PH), std: std(S_PH), trend: trend(S_PH) },
    soilEc: { mean: mean(S_EC), std: std(S_EC), trend: trend(S_EC) },
    soilN: { mean: mean(S_N), std: std(S_N), trend: trend(S_N) },
    soilP: { mean: mean(S_P), std: std(S_P), trend: trend(S_P) },
    soilK: { mean: mean(S_K), std: std(S_K), trend: trend(S_K) },
    soilTemp: { mean: mean(S_T), std: std(S_T), trend: trend(S_T) },
    soilHum: { mean: mean(S_H), std: std(S_H), trend: trend(S_H) },
    n: records.length,
  };
}
    ...
    useRecords = [
      {
        ...
        S_PH: latestReadings.phTanah,
        S_EC: latestReadings.ecTanah,
        S_N: latestReadings.nTanah,
        S_P: latestReadings.pTanah,
        S_K: latestReadings.kTanah,
        S_T: latestReadings.suhuTanah,
        S_H: latestReadings.kelembapanTanah,
      },
    ];
  }
...
function mlExportDataset() {
  ...
  let csv =
    'Timestamp,Suhu_C,Kelembapan_RH,EC_uS,TDS_ppm,SuhuAir_C,Soil_pH,Soil_EC,Soil_N,Soil_P,Soil_K,Soil_Temp,Soil_Hum\n';
  records.forEach((r) => {
    const d = new Date(r.t).toISOString();
    csv += `${d},${r.T ?? ''},${r.H ?? ''},${r.E ?? ''},${r.D ?? ''},${r.W ?? ''},${r.S_PH ?? ''},${r.S_EC ?? ''},${r.S_N ?? ''},${r.S_P ?? ''},${r.S_K ?? ''},${r.S_T ?? ''},${r.S_H ?? ''}\n`;
  });
